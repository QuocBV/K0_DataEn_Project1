...
-- Derivatives trading account. customer_code / broker_code are logical references to SSI_Common
...
CREATE TABLE raw.account (
    ...
    source_system     VARCHAR(50)   NOT NULL CONSTRAINT df_account_source_system DEFAULT ('DERIVATIVES'),
...
-- Partitioned monthly by balance_date via the shared ps_derivatives_monthly scheme.
CREATE TABLE raw.account_balance_daily (
    ...
    source_system      VARCHAR(50)   NOT NULL CONSTRAINT df_acct_balance_source_system DEFAULT ('DERIVATIVES'),
...
) ON ps_derivatives_monthly(balance_date);
GO

-- End-of-day open position per account, per contract.
-- Partitioned monthly by position_date via the shared ps_derivatives_monthly scheme.
CREATE TABLE raw.position_daily (
    ...
    source_system      VARCHAR(50)   NOT NULL CONSTRAINT df_position_daily_source_system DEFAULT ('DERIVATIVES'),
    ingested_at        DATETIME2(3)  NOT NULL CONSTRAINT df_position_daily_ingested_at DEFAULT (SYSUTCDATETIME()),
    CONSTRAINT pk_position_daily PRIMARY KEY CLUSTERED (account_id, contract_id, position_date),
    CONSTRAINT fk_position_daily_account FOREIGN KEY (account_id) REFERENCES raw.account(account_id),
    CONSTRAINT fk_position_daily_contract FOREIGN KEY (contract_id) REFERENCES raw.derivative_contract(contract_id),
    CONSTRAINT chk_position_daily_side CHECK (position_side IN ('LONG', 'SHORT'))
) ON ps_derivatives_monthly(position_date);
GO

-- Daily margin balance and maintenance ratio per account - drives margin-call risk reporting.
-- Partitioned monthly by loan_date via the shared ps_derivatives_monthly scheme.
CREATE TABLE raw.margin_loan_daily (
    ...
    source_system             VARCHAR(50)   NOT NULL CONSTRAINT df_margin_loan_source_system DEFAULT ('DERIVATIVES'),
...
) ON ps_derivatives_monthly(loan_date);
GO
