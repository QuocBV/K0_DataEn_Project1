...
-- Shared partition function/scheme for equity_trade and the daily account/position snapshot
...
CREATE PARTITION FUNCTION pf_equity_monthly (DATE)
...
CREATE PARTITION SCHEME ps_equity_monthly
AS PARTITION pf_equity_monthly ALL TO ([PRIMARY]);
GO
...
BEGIN
    ...
    ALTER PARTITION SCHEME ps_equity_monthly NEXT USED [PRIMARY];
    ALTER PARTITION FUNCTION pf_equity_monthly() SPLIT RANGE (@boundary);
END;
GO
