{{ config(materialized='incremental', unique_key=['account_id', 'fund_id', 'position_date'], on_schema_change='append_new_columns') }}
...
with flattened as (
    select
        ...
        raw_data:avg_cost_nav::number(18,4) as avg_cost_nav,
        raw_data:market_value::number(20,2) as market_value,
        _loaded_at
    from {{ source('raw_oef', 'position_daily_raw') }}
    {% if is_incremental() %}
...
qualify row_number() over (
    partition by account_id, fund_id, position_date
