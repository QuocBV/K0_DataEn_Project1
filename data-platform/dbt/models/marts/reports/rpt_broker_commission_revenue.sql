-- Report 1: Hoa hong & doanh so theo Moi gioi/CTV, roll up-able to Phong ban/Chi nhanh via
-- dim_broker. One row per (trade_date, broker_code, product_type); a BI tool aggregates further by
-- week/month/quarter (join dim_date) or by department/branch (join dim_broker).
select
    t.trade_date,
    t.product_type,
    t.broker_code,
...
left join {{ ref('dim_fee_schedule') }} fs
