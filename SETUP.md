...
- SQL Server instance
- AWS S3 buckets: `company-data` và `company-report`
...
Tạo: Sources (MSSQL ×4 + HR API) → Destination S3 (`company-data/raw/{source}/{table}/{date}/`)
...
SELECT * FROM company_data.gold.dim_customer
FOR VERSION AS OF TIMESTAMP '2027-03-15 10:00:00';

-- Báo cáo từ Commission Engine
SELECT * FROM company_report.reporting.rpt_broker_commission_revenue;
```
